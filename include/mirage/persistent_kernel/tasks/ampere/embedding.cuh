...
__device__ __forceinline__ void
  ...
  for (int i = threadIdx.x; i < BATCH_SIZE * OUT_DIM; i += WORKER_NUM_THREADS) {
    // int idx = i / OUT_DIM;
    int off = i % OUT_DIM;
...
#pragma unroll
  for (int batch_idx = 0; batch_idx < BATCH_SIZE; batch_idx++) {
    ...
    if (wordIdx >= 0) {
#pragma unroll
      for (int i = threadIdx.x; i < CHUNK_SIZE; i += WORKER_NUM_THREADS) {
      ...
      for (int i = threadIdx.x; i < CHUNK_SIZE;
           i += WORKER_NUM_THREADS) { // writing 0 to output
