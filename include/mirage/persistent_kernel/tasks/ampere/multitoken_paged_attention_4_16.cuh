...
#pragma unroll
  ...
  int page_idx_0 = page_indices_g[0];
#pragma unroll
  for (int chunk_idx = tid;
...
#pragma unroll
  ...
  for (int iter = 0; iter < num_iters; iter++) {
    int next_iter_len = iter + 1 < num_iters
                            ...
                            : 0;
    if (next_iter_len > 0) {
      int page_idx = page_indices_g[cp_finished_seq_len / PAGE_SIZE];
#pragma unroll
      for (int chunk_idx = tid;
           chunk_idx < curr_iter_len * HEAD_DIM / CP_CHUNK_SIZE;
           chunk_idx += NUM_THREADS) {
...
#pragma unroll
    ...
    // update the KV Cache
    if (kv_tokens_to_process > 0) {
      int page_idx = page_indices_g[first_kv_token_to_process / PAGE_SIZE];
...
template <typename T,
          ...
          int PAGE_SIZE,
          int MAX_TOKENS = 8>
__device__ __forceinline__ void multitoken_paged_attention_task_impl_4_16_blocked(
    void const *qkv_ptr,
    void *paged_k_cache_ptr,
    void *paged_v_cache_ptr,
    void *output_ptr,
  ...
  int const total_num_tokens = last_token_pos - first_token_pos;
...
#pragma unroll
  ...
  for (int token_offset = 0; token_offset < total_num_tokens;
    ...
    T const *__restrict__ d_q =
        ...
        (first_token_pos + token_offset) * QKV_STRIDE;
    T const *__restrict__ d_k = d_q + NUM_QO_PER_KV * HEAD_DIM;
    T const *__restrict__ d_v = d_k + HEAD_DIM;
    T *__restrict__ d_output =
        reinterpret_cast<T *>(output_ptr) +
        (first_token_pos + token_offset) * O_STRIDE;
...
#pragma unroll
  for (int chunk_idx = tid;
    ...
    if (dst_row + cp_finished_seq_len < seq_len - total_num_tokens) {
      ...
      // load from QKV
      int src_row =
          dst_row + cp_finished_seq_len - (seq_len - total_num_tokens);
      load_smem(k_buffer_smem(dst_row, col), k_dmem(src_row, col));
      load_smem(v_buffer_smem(dst_row, col), v_dmem(src_row, col));
    }
  }
...
#pragma unroll
    ...
    int kv_tokens_to_process = min(
        curr_iter_len,
        max(iter * KV_TILE_SIZE + curr_iter_len - (seq_len - total_num_tokens),
            0));
    int first_kv_token_to_process =
        iter * KV_TILE_SIZE + curr_iter_len - kv_tokens_to_process;
    if (qk_norm) {
      ...
      if (iter == 0) {
        rms_norm<T, QOSmem, NUM_QO_PER_KV, HEAD_DIM>(
            ...
            rope,
            static_cast<T const *>(cos_ptr) +
                (seq_len - total_num_tokens + token_offset) * HEAD_DIM,
            static_cast<T const *>(sin_ptr) +
                (seq_len - total_num_tokens + token_offset) * HEAD_DIM);
      }
      // K norm
...
#pragma unroll
        for (int token_idx = 0; token_idx < num_tokens; token_idx++) {
          ...
          rotary_embedding<T, QOSmem, NUM_QO_PER_KV, 1, HEAD_DIM>(
              q_smem,
              static_cast<T const *>(cos_ptr) +
                  (token_idx + seq_len - total_num_tokens + token_offset) *
                      HEAD_DIM,
                  static_cast<T const *>(sin_ptr) +
                  (token_idx + seq_len - total_num_tokens + token_offset) *
                      HEAD_DIM,
      ...
      for (int frag_idx = 0; frag_idx < 8; frag_idx++) {
        ...
        bool is_valid =
            ...
            (col + iter * KV_TILE_SIZE <=
             token_idx + seq_len - total_num_tokens);
        x_frag_f[m][frag_idx] = is_valid ? x_frag_f[m][frag_idx] : -inf;
        m_local[m][(frag_idx & 0x3) >> 1] =
            max(m_local[m][(frag_idx & 0x3) >> 1], x_frag_f[m][frag_idx]);
      }
